        ...
        implements ColumnWriter
{
    ...
    private List<ParquetDataOutput> getDataStreams()
            throws IOException
    {
        ImmutableList.Builder<ParquetDataOutput> outputStreams = ImmutableList.builder();
        if (valueCount > 0) {
            flushCurrentPageToBuffer();
        }
        ...
        if (dictionaryPage != null) {
            ...
            outputStreams.add(pageHeader);
            outputStreams.add(pageData);
        ...
        outputStreams.add(createDataOutput(compressedOutputStream));
        return outputStreams.build();
    }
