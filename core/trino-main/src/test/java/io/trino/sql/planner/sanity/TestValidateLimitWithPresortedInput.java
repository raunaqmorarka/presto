        ...
        extends BasePlanTest
{
    ...
    public void testValidateSuccessful()
    {
        validatePlan(
                p -> {
                    List<Symbol> orderBy = ImmutableList.of(p.symbol(COLUMN_NAME_A, VARCHAR), p.symbol(COLUMN_NAME_C, VARCHAR));
                    OrderingScheme inputOrdering = new OrderingScheme(
                            orderBy,
                            Maps.toMap(orderBy, Functions.constant(ASC_NULLS_FIRST)));
                    return p.limit(
                            10,
                            ImmutableList.of(),
                            true,
                            Optional.of(inputOrdering),
                            p.tableScan(
                                    MOCK_TABLE_HANDLE,
    ...
    @Test(expectedExceptions = VerifyException.class, expectedExceptionsMessageRegExp = "Limit with input ordering is not sorted by \\[S↑→\\(col_a\\)]")
    public void testValidateFailed()
    {
        validatePlan(
                p -> {
                    List<Symbol> orderBy = ImmutableList.of(p.symbol(COLUMN_NAME_A, VARCHAR));
                    OrderingScheme inputOrdering = new OrderingScheme(
                            orderBy,
                            Maps.toMap(orderBy, Functions.constant(ASC_NULLS_LAST)));
                    return p.limit(
                            10,
                            ImmutableList.of(),
                            true,
