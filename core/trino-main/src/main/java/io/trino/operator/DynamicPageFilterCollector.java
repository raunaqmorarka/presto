...
{
    @GuardedBy("this")
    private TupleDomain<Symbol> collectedTupleDomain = TupleDomain.all();
    ...
    private ListenableFuture<?> collectDynamicFilterAsync(SettableFuture<?> completionFuture)
    {
        if (dynamicFilter.isComplete()) {
            TupleDomain<Symbol> tupleDomain = translateSummaryIntoTupleDomain(dynamicFilter.getCurrentPredicate(), columnHandles);
            combineFilters(tupleDomain);
    ...
    private synchronized void combineFilters(TupleDomain<Symbol> tupleDomain)
    {
        TupleDomain<Symbol> intersected = collectedTupleDomain.intersect(tupleDomain);
        if (!intersected.equals(collectedTupleDomain)) {
            collectedTupleDomain = intersected;
            RowExpression combinedExpression = new SpecialForm(
                    SpecialForm.Form.AND,
                    BOOLEAN,
                    staticFilter.get(),
                    converter.toRowExpression(domainTranslator.toPredicate(collectedTupleDomain)));
