...
public class ExpressionCompiler
{
            ...
            Optional<DynamicPageFilterCollectorFactory> dynamicPageFilterCollectorFactory)
    {
        Optional<Supplier<PageFilter>> filterFunctionSupplier = filter.map(expression -> {
            Supplier<PageFilter> pageFilterSupplier = pageFunctionCompiler.compileFilter(expression, classNameSuffix);
            if (dynamicPageFilterCollectorFactory.isPresent()) {
                DynamicPageFilterCollector dynamicFilterCollector = dynamicPageFilterCollectorFactory.get().create(pageFunctionCompiler, classNameSuffix);
                dynamicFilterCollector.startCollectDynamicFilterAsync();
                return () -> new DynamicPageFilter(Optional.of(pageFilterSupplier.get()), dynamicFilterCollector);
            }
            else {
